...
df = pd.DataFrame({"domain":["ex1.ans1.tld1","ex2.ans1.tld1","ex1.ans2.tld1","ex2.ans2.tld1",
			     "ex1.ans3.tld2","ex2.ans3.tld2","ex1.ans4.tld2","ex2.ans4.tld2"]})
'''
...
df = df.assign(recursive_resolver = "172.20.4.3")
...
def query_rs_server(domain, rns):
    try:
#       cmd = 'dig {} @{}' .format(tld, rns)
	cmd = 'dig {} @{} | grep ";; Query time"'.format(domain, rns)
        proc=subprocess.Popen(cmd,stdout=subprocess.PIPE,shell=True)
        out,err=proc.communicate()
        return int(out.decode().split(" ")[3])
    except:
        return -1
...
#       cmd = 'dig {} @{}' .format(domain, tld_server)
	cmd = 'dig {} @{} | grep ";; Query time"'.format(domain, tld_server)
        proc=subprocess.Popen(cmd,stdout=subprocess.PIPE,shell=True)
        out,err=proc.communicate()
        return int(out.decode().split(" ")[3])
    except:
...
#       cmd = 'dig {} @{}' .format(domain, ans_server)
	cmd = 'dig {} @{} | grep ";; Query time"'.format(domain, ans_server)
        proc=subprocess.Popen(cmd,stdout=subprocess.PIPE,shell=True)
        out,err=proc.communicate()
        return int(out.decode().split(" ")[3])
    except:
...
df.to_csv("/etc/dnsmasq.d/test/result_6paths_15.csv")
